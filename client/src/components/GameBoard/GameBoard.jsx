...
const InitializedBoardCriteria = (colIdx, rowIdx, cell) => {
  return {
...
const renderUpdate = (boardUpdate) => {
  const col = randomNum();
  const row = randomNum();
...
class GameBoard extends Component {
    ...
    arr.forEach((colArr, rowIdx) =>
      colArr.forEach((cell, colIdx) =>
        newArr.push(
          React.createElement(
            "div",
            InitializedBoardCriteria(colIdx, rowIdx, cell)
          )
        )
      )
    );
  ...
  render() {
    let leaderBoard = this.renderLeaderBoard();

    return (
      <div>
        <div
          className="main"
          hidden={this.props.history.location.pathname === "/leaderboard"}
        >
          ...
          <Swipeable
            onSwiped={(e) => {
              let changingBoard = this.state.board.slice();
              const arrayBefore = JSON.stringify(changingBoard);
              let movedBoard = this.moves[e.dir.toLowerCase()](changingBoard);
              let check = JSON.stringify(movedBoard);
              //the if statement checks if a move can be made, otherwise nothing happens
              if (arrayBefore !== check) {
                this.setState({ board: renderUpdate(movedBoard) });
